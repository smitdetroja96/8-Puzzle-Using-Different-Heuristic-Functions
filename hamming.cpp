...
//----------------- To find hamming distance of board ----------------//

ll hamming(board curr_board, ll moves)
{
    ll totalHammingDistance = moves;
    FOR(i,0,n)
    FOR(j,0,n)
    if(curr_board[i][j]!=end_board[i][j] && curr_board[i][j]!=0)
...
struct cmp
{
    ...
    {
        ld x = hamming(a.first,a.second);
        ld y = hamming(b.first,b.second);
        return x > y;
    }
};
